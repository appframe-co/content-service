...
import { TOptions, TSectionInput, TSection, TSectionModel, TContentModel } from '@/types/types';
...
export default async function CreateSection(sectionInput: TSectionInput): Promise<{section: TSection|null, userErrors: any}> {
    try {
        const {projectId, contentId, userId, parentId, ...sectionBody} = sectionInput;
                            ...
                            for (const schemaData of schemaDataBody) {
                                ...
                                if (schemaData.type === 'single_line_text' || schemaData.type === 'multi_line_text' || schemaData.type === 'rich_text') {
                                    ...
                                    if (Array.isArray(options.unique) ? options.unique[0] : options.unique) {
                                        const isUniquie: boolean|null = await checkUnique(valueValue, {projectId, contentId, subject: 'section', key: 'doc.'+schemaData.key});
                                        if (isUniquie === false) {
        ...
        const {errors: errorsDB, data: savedData} = await (async (data) => {
            try {
                ...
                const parentSection: TSectionModel|null = await Section.findOne({_id: parentId, projectId, contentId, userId});
                const pId = parentSection && parentSection.id;

                const section: TSectionModel|null = await Section.create({
                    ...data.section, parentId: pId, 
                    projectId, contentId, userId, 
